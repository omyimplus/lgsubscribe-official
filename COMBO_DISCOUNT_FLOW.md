...
| 2026-06-25 | **มีมัดจำ** → เดือนที่ 1 หัก combo ทันที · เดือนที่ 2 ขึ้นไปหัก % เฉพาะงวดนั้น (ไม่เลื่อนส่วนลดจากเดือนแรก) · **ไม่มีมัดจำ** → ยังใช้ `defer_rate` เดิม |
...
4. ต่อบิล `b` ของแต่ละรายการ (ให้ `base(b)` = ราคาแผนงวดนั้น, `pct` = combo %):
   ...
   - **บิล 2** → ชำระ `base(2) − round(pct% × base(1)) − round(pct% × base(2))`
   - **บิล 3+** → ชำระ `base(b) − round(pct% × base(b))`

**มีมัดจำ (`immediate_rate`):**

- วันทำรายการ = มัดจำ (`advance_amount`) — ไม่หัก combo
- **เดือนที่ 1 (บิลแผน 1)** → หัก combo ทันที: `base(1) − round(pct% × base(1))`
- **เดือนที่ 2 ขึ้นไป** → หักเฉพาะ % ของงวดนั้น: `base(b) − round(pct% × base(b))` — **ไม่รวมส่วนลดที่เลื่อนจากเดือนแรก**
...
### 4.2 มีมัดจำ — `immediate_rate`

```
ทุกบิล b ≥ 1 → charged = base(b) − round(pct%×base(b))
```

- มัดจำ (`advance_amount`) ไม่หัก combo
- เดือนที่ 1 หัก combo ทันที — ไม่เลื่อนไปรวมในเดือนที่ 2
